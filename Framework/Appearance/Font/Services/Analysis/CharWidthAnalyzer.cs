...
namespace TM.Framework.Appearance.Font.Services
{
    ...
    public class CharWidthAnalyzer
    {
        ...
        public CharWidthReport AnalyzeFont(string fontName, double fontSize, double pixelsPerDip = 0, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                report.Items.Add(CheckDigitWidths(fontName, fontSize, pixelsPerDip, ct));

                ct.ThrowIfCancellationRequested();
                report.Items.Add(CheckLetterWidths(fontName, fontSize, pixelsPerDip, ct));

                ct.ThrowIfCancellationRequested();
                report.Items.Add(CheckSpecialCharWidths(fontName, fontSize, pixelsPerDip, ct));

                ct.ThrowIfCancellationRequested();
                report.Items.Add(CheckCJKWidths(fontName, fontSize, pixelsPerDip, ct));
        ...
        private WidthCheckItem CheckDigitWidths(string fontName, double fontSize, double pixelsPerDip, CancellationToken ct)
        {
            var digits = "0123456789";
            var widths = MeasureCharacterWidths(fontName, fontSize, digits, pixelsPerDip, ct);
        ...
        private WidthCheckItem CheckLetterWidths(string fontName, double fontSize, double pixelsPerDip, CancellationToken ct)
        {
            var letters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
            var widths = MeasureCharacterWidths(fontName, fontSize, letters, pixelsPerDip, ct);
        ...
        private WidthCheckItem CheckSpecialCharWidths(string fontName, double fontSize, double pixelsPerDip, CancellationToken ct)
        {
            var specialChars = "[]{}()<>";
            var widths = MeasureCharacterWidths(fontName, fontSize, specialChars, pixelsPerDip, ct);
        ...
        private WidthCheckItem CheckCJKWidths(string fontName, double fontSize, double pixelsPerDip, CancellationToken ct)
        {
            var cjkChars = "中文汉字测试";
            var widths = MeasureCharacterWidths(fontName, fontSize, cjkChars, pixelsPerDip, ct);
            ...
            var latinWidth = MeasureCharacterWidths(fontName, fontSize, "a", pixelsPerDip, ct).FirstOrDefault();
        ...
        private List<double> MeasureCharacterWidths(string fontName, double fontSize, string characters, double pixelsPerDip, CancellationToken ct)
        {
        ...
        private string GenerateSummary(CharWidthReport report)
        {
            ...
            {
                if (i.Result == WidthCheckResult.Pass) passCount++;
                else if (i.Result == WidthCheckResult.Warning) warnCount++;
                else if (i.Result == WidthCheckResult.Fail) failCount++;
            }

...
namespace TM.Framework.Appearance.Font.Services
{
    ...
    public class EditorFontPresetService
    {
        ...
        private readonly Task<HashSet<string>> _installedFontsTask;

        public EditorFontPresetService()
        {
            _installedFontsTask = Task.Run(LoadInstalledFonts);
        ...
        {
            if (!_installedFontsTask.IsCompletedSuccessfully)
                return false;
            return _installedFontsTask.Result.Contains(fontName);
        }
        ...
        {
            if (!_installedFontsTask.IsCompletedSuccessfully) return;

...
namespace TM.Framework.UI.Helpers
{
    ...
    {
        private const string ConfirmMessage = "当前业务存在未保存的AI生成会话。切换业务将结束本次会话，可能导致后续生成的连贯性下降。\n\n是否仍要切换？";
        private const string ConfirmTitle = "切换业务确认";

        public static async Task<bool> TryConfirmAndEndDirtyBusinessSessionAsync(string? currentBusinessKey)
        {
            try
            {
                ...
                var confirmed = await StandardDialog.ShowConfirmAsync(ConfirmMessage, ConfirmTitle);
